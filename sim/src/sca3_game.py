...
def query_max_k(): return max_open_charger

def query_chargers(): return total_charger
...
def getMaxRadiation(switch_sequence):
    ...
    if intersection_flag:
        intersectionTraverse() # 如果map里面没有交点，先去算一遍
...
def get_seq(action1,action2):
    i=total_charger-1
    state= [0]*total_charger 
    while action1:
        if(action1 & 1):
            state[i]=1
        action1 >>= 1
...
def get_payoff():
    action_num=(2**total_charger)-1
    agent_num=2
    payoff = np.zeros(tuple([agent_num] + [action_num] * agent_num))
    for i in range(action_num):
        for j in range(action_num):
            payoff[0][i][j], payoff[1][i][j]= sca3_step(get_seq(i,j))
    return payoff